...
void extractLD(hPtr header, int v) {
  ...
  if (S_ISLNK(mode)) {
    ...
    *(strrchr(dirname, '/')) = '\0';
    ...
    setFileSettings(header, dirname);
  }
}
...
void extractFile(int tarFD, hPtr header, int v) {
  ...
  while ((total_bytes_written < size) &&
    ...
    bytes_written =
        write(outFileFD, buffer, MIN(bytes_read, (size - total_bytes_written)));
...
void extract_subset(char *tarfile, int tarFD, char **paths, int num_paths,
                    int v) {
  int num, i;
  hPtr header = malloc(HEADER_SIZE);
  ...
  while ((num = read(tarFD, header, BLOCK_SIZE)) > 0) {
    ...
    for (i = 0; i < num_paths; i++) {
      if (strcmp(header->name, paths[i]) == 0) {
...
void extractArchive(char *tarfile, char **paths, int num_paths, int v) {
  ...
  if (num_paths > 0) {
    ...
    extract_subset(tarfile, tarFD, paths, num_paths, v);
#ifdef TEST
